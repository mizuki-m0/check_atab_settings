function checkSettings (check_items) {
    ...
    var bt_vo_num = 
    Number(check_items["bt_voice_A"] || 0)
    + Number(check_items["bt_voice_B"] || 0)
    + Number(check_items["bt_voice_C"] || 0)
    + Number(check_items["bt_voice_D"] || 0)
    + Number(check_items["bt_voice_E"] || 0)
    + Number(check_items["bt_voice_F"] || 0); 

    if ("bt_voice_A" in check_items){
        var check_bt_voice_A = Probability_calc(bt_vo_num, check_items["bt_voice_A"], prob_bt_voice_A);
        check_all = multi_array(check_all, check_bt_voice_A)
    }
    if ("bt_voice_B" in check_items){
        var check_bt_voice_B = Probability_calc(bt_vo_num, check_items["bt_voice_B"], prob_bt_voice_B);
        check_all = multi_array(check_all, check_bt_voice_B)
    }
    if ("bt_voice_C" in check_items){
        var check_bt_voice_C = Probability_calc(bt_vo_num, check_items["bt_voice_C"], prob_bt_voice_C);
        check_all = multi_array(check_all, check_bt_voice_C)
    }
    if ("bt_voice_D" in check_items){
        var check_bt_voice_D = Probability_calc(bt_vo_num, check_items["bt_voice_D"], prob_bt_voice_D);
        check_all = multi_array(check_all, check_bt_voice_D)
    }
    if ("bt_voice_E" in check_items){
        var check_bt_voice_E = Probability_calc(bt_vo_num, check_items["bt_voice_E"], prob_bt_voice_E);
        check_all = multi_array(check_all, check_bt_voice_E)
    }
    if ("bt_voice_F" in check_items){
        var check_bt_voice_F = Probability_calc(bt_vo_num, check_items["bt_voice_F"], prob_bt_voice_F);
        check_all = multi_array(check_all, check_bt_voice_F)
    }
